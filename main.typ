...
#show: project.with(
  ...
  abstract: [
    Пояснювальна записка до курсової роботи: 37 с., 5 рис., 1 табл., 2 додатки, 4 джерел інформації.
...
У першій частині роботи розглядаються базові концепції патернів проектування, їх значення для розробки програмного забезпечення, історія створення та розвиток патерну Strategy, а також аналізуються проблеми, які він допомагає вирішити.  

Друга частина присвячена опису основних елементів патерну Strategy, включаючи детальний розгляд класів, інтерфейсів і методів, які задіяні в його реалізації.  

Третя частина містить відомості про практичне використання патерну Strategy, його переваги та недоліки, а також взаємозв’язки з іншими патернами проектування.  
...
Критично важливо, щоб більшість програм сьогодні використовували зовнішні бібліотеки для вирішення різноманітних задач — від роботи з базами даних до обробки зображень чи навіть машинного навчання. За допомогою pip програмісти можуть швидко інтегрувати готові рішення та фокусуватися на бізнес-логіці, знижуючи час, необхідний для розробки та тестування. Оскільки Python має величезну екосистему бібліотек, правильне керування залежностями через pip є критично важливим для підтримки стабільності та безпеки програмного забезпечення.
...
Патерн проектування Strategy належить до групи поведінкових патернів і був вперше описаний в @Gamma1994DesignPatterns. Книга стала основним джерелом для розвитку об'єктно-орієнтованого програмування та патернів проектування, що дозволило стандартизувати багато рішень, які повторюються в розробці програмного забезпечення.
...
Python Package:
  - *rich* — потужна бібліотека для виведення кольорових та стилізованих текстів у термінал. Вона підтримує виведення таблиць, прогрес-барів, візуальних компонентів, що робить взаємодію з користувачем значно зручнішою та інтуїтивно зрозумілою. Інформація про *rich* @rich_github
#pagebreak()
...
Графічне представлення ієрархії класів на @diagram_class_project
...
Необхідні версії:\
  - Python 3.6 і вище.
  - Бібліотека rich.
Для отримання проекту виконайте команду:
```bash
git clone https://github.com/JordanOleg/zomboid.git
```
Пряме посилання на проект @github_source_code\
...
3. Передає віддесерилізовані дані в клас Zomboid який виведе на екран таблицю об’єктів @table_models_result

#align(center, [_Data Models_])
#figure(caption: [Результат програми], kind: table, 
table(columns: 5, stroke: 2pt,
[*id*], [*name*], [*type_object*], [*condition*], [*amount*], 
[1], [Hammer], [Tool], [Mind], [10], 
[2], [Nails], [Fasteners], [Good], [450], 
[3], [Nails], [Fasteners], [Bad], [100],
[4], [Screwdriver], [Tool], [Used], [25],
[5], [Bolt], [Fasteners], [New], [200], 
[6], [Hammer], [Tool], [Mint], [50],
[7], [Nails], [Fasteners], [Used], [300],
[8], [Drill], [Tool], [Good], [15])
)<table_models_result>
...
#pagebreak()
#heading(level: 1, "Перелік посилань", numbering: none)
...
#pagebreak()
#heading("Додаток А", numbering: none)
#image("photo/photo_presentations/present_front.jpg")
...
#pagebreak()
#heading("Додаток Б", numbering: none)
```python
import csv
